...
with open(args.input_path, 'r', encoding='utf-8') as f:
    for line in f.readlines():
        ...
        if l:
            l = line.split('\t')
            sentences[-1].setdefault('tokens', [])
            sentences[-1].setdefault('extra', [])
            sentences[-1]['tokens'].append(l[1])
            sentences[-1]['extra'].append([l[0]] + l[6:])
        else:
            if sentences[-1] != {}:
                sentences.append({})
...
for idx in range(len(target)):
    parsed.append({
        'raw_tokens': dataset[idx]['raw_tokens'],
        'extra': sentences[idx]['extra']
    })
...
for i in parsed:
    ...
    for r in list(zip(*[l for l in list(i.values())])):
        row = tpl.format(
            ...
            lemma=r[2],
            pos=r[3],
            features=parse_feat(r[4:], features),
            head=r[1][1],
            dep=r[1][2])
        f.write(row)
        idx += 1
